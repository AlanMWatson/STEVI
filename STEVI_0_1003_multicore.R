#########################################################################
##  STEVI (STat Expression with Virus Infection) Ver: 0.1003 Multicore ##
##  2014-08-13 AMW                                                     ##
#########################################################################
...
colocalization <- function(selection="YFV", wholecell="STAT", colocBrush = 25, colocThresh = 0.75, writeimage=FALSE, writereport=FALSE, ...){
          ...
          cmask <- cmask
          selectionimagespos <- selectionimagespos
          
          cell <<- foreach(i = 1:n, .combine="cbind") %:%
                    
                    foreach(c = 1:max(cmask), .combine="rbind") %dopar%{
                              
                              celltmp <- matrix(data=NA, nrow=1, ncol=1)
...
statQuant <- function(hipassQuant = FALSE, ...){
          ...
          cmask <- cmask
          cellNum <- colSums(!is.na(cell))
                    
          imageDat <<- foreach(u = 1:imgeNum, .combine="rbind") %:%
                    
                    foreach(i = 1:cellNum[u], .combine="rbind") %dopar% {
