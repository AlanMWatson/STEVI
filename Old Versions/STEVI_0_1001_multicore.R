#########################################################################
##  STEVI (STat Expression with Virus Infection) Ver: 0.1001 multicore ##
##  2014-08-12 AMW                                                     ##
#########################################################################
...
#           imageDat <<- as.data.frame(imageDat)
          ...
          dataList <<- foreach(u = 1:imgeNum)  %dopar% {
                    
                    
                    cellNum <- sum(!is.na(cell[,u]))
                    
                    tmp <- matrix(nrow = cellNum, ncol = 9)
                    imageDatColNames <- c("imagenum", "treatment", "cell.number", "infected", "stat.mean", "stat.sum", "yfv.mean", 
                                          "yfv.sum", "cell.pixel.area")
                    ...
                    for(i in 1:cellNum){
                              ...
                              if(cell[i, u] == 0){
                                        ...
                                        tmp$infected[r] <- "negative"
                              }
                              else if(cell[i, u] == 1){
                                        
                                        tmp$infected[r] <- "positive"
                              }
                              
                              tmp$stat.mean[r] <-  mean(whole)
                              tmp$stat.sum[r] <- sum(whole)
                              tmp$yfv.mean[r] <- mean(select)
                              tmp$yfv.sum[r] <- sum(select)
                              tmp$cell.pixel.area[r] <- sum(cellLocation)
          ...
          imageDat <<- dataList[[1]]
          for(i in 2:imgeNum){
                    imageDat <<- rbind(imageDat, dataList[[i]])
          }
